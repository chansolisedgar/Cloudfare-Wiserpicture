...
async function syncWithMailchimp(email, name, tags, env) {
...
function parseModulesFromCustomData(customData) {
  if (!customData || !customData.modules) return [1];
...
export async function onRequestPost({ request, env }) {
  try {
    ...
    if (eventName === 'order_created') {
      ...
      await syncWithMailchimp(email, name, tags, env);
      ...
      const supabaseAnon = createClient(
        env.SUPABASE_URL,
        env.SUPABASE_ANON_KEY || 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InF3Y2FnZGxzbGt4cnpxbmd5c3RqIiwicm9sZSI6ImFub24iLCJpYXQiOjE3Nzg3OTkyNDMsImV4cCI6MjA5NDM3NTI0M30.rI6-hkEDvs9m_TJDVAc3eaPygo_1GRNUVyg9QbTvNJc'
      );

      async function findUserByEmail(targetEmail) {
        let page = 1;
        while (page <= 10) {
          const { data: { users }, error } = await supabaseAdmin.auth.admin.listUsers({ page: page, perPage: 1000 });
      ...
      return new Response(JSON.stringify({ success: true, message: `User provisioned` }), { status: 200, headers: { 'Content-Type': 'application/json' }});
    }
