...
export async function onRequestPost({ request, env }) {
  try {
    ...
    if (!apiKey || !audienceId) {
      console.error('Missing Mailchimp config');
    ...
    if (!memberResponse.ok) {
      ...
      console.error('Error suscribiendo:', errorText);
      return new Response(JSON.stringify({ error: 'Error al suscribir en Mailchimp', details: errorText }), { status: memberResponse.status });
    }
    ...
    console.error('Error en la función subscribe:', error.message);
